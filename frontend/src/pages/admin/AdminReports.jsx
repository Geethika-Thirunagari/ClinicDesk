  ...
  return (
    <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="space-y-6 p-4 lg:p-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl lg:text-3xl font-extrabold text-slate-800 tracking-tight">System Reports</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {reportTypes.map((type) => (
            <motion.div key={type.id} whileHover={{ y: -2 }} onClick={() => { setSelectedType(type); setShowModal(true); }}
              className="bg-white/60 backdrop-blur-xl border border-white/40 shadow-sm hover:shadow-md rounded-2xl p-5 cursor-pointer transition-all flex items-start gap-4">
      ...
      <div className="bg-white/60 backdrop-blur-xl border border-white/40 shadow-sm rounded-2xl p-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <button className="flex items-center gap-2 px-3 py-1.5 text-sm font-semibold text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-lg transition-colors">
              ...
              {recentReports.map((report) => (
                <tr key={report.id} className="border-b border-slate-100 hover:bg-slate-50/50 transition-colors group">
                  ...
                  <td className="py-4 text-right">
                    <button className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-lg transition-colors">
        ...
        {showModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} 
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
              onClick={(e) => e.stopPropagation()} className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-8">
              
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-bold text-slate-800">Generate Report</h2>
                <button onClick={() => !isGenerating && setShowModal(false)} className="p-2 rounded-lg hover:bg-slate-100 text-slate-400 disabled:opacity-50" disabled={isGenerating}>
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="space-y-5">
                <div>
                  ...
                  <select value={selectedType.id} onChange={(e) => setSelectedType(reportTypes.find(t => t.id === e.target.value))} disabled={isGenerating}
                    className="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm font-semibold outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="date" className="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50" disabled={isGenerating} />
                  </div>
                  <div>
                ...
                <div>
                  ...
                  <div className="flex gap-3">
                    {['PDF', 'CSV', 'Excel'].map(format => (
                      <label key={format} className="flex-1 flex items-center justify-center gap-2 p-3 border border-slate-200 rounded-xl cursor-pointer hover:bg-slate-50 transition-colors">
