        ...
        <div className="flex flex-col md:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input type="text" placeholder="Search by name or patient ID..." value={search} onChange={(e) => setSearch(e.target.value)}
              className="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm text-[#0a1a0f] placeholder:text-slate-400 :text-slate-500 outline-none focus:ring-2 focus:ring-blue-500 transition-all" />
          </div>
            ...
            <tbody>
              {filtered.map((pt, i) => (
                <motion.tr key={pt.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.05 }}
                  className="border-b border-slate-100 hover:bg-blue-50/30 :bg-blue-900/20 transition-colors group cursor-pointer">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-semibold text-sm text-[#0a1a0f] ">{pt.name}</p>
