  ...
  return (
    <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="space-y-6 p-4 lg:p-8 min-h-screen">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl lg:text-3xl font-extrabold text-[#0a1a0f] tracking-tight">Welcome back, Alice 👋</h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <motion.button key={action.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 * i }}
            ...
            className="finai-card p-5 text-left hover:shadow-md transition-all group">
            <div className={cn("w-12 h-12 rounded-xl flex items-center justify-center mb-3", action.color)}>
              <action.icon size={22} className="text-white" />
            </div>
            <p className="font-bold text-sm text-[#0a1a0f] ">{action.label}</p>
          </motion.button>
        ))}
      </div>
          ...
          <div className="space-y-4">
            {upcomingAppointments.map((apt, i) => (
              <motion.div key={apt.id} initial={{ opacity: 0, x: -15 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 * i }}
                className="flex items-center justify-between p-4 rounded-xl bg-slate-50/80 border border-slate-100 hover:bg-slate-100/80 :bg-slate-800 transition-all group">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-full bg-gradient-to-tr from-blue-100 to-indigo-100 flex items-center justify-center font-bold text-blue-600 ">
                    {apt.doctor.charAt(4)}
                  </div>
                  <div>
                    <h4 className="font-bold text-sm text-[#0a1a0f] ">{apt.doctor}</h4>
                    <p className="text-xs text-slate-500">{apt.specialty} • {apt.mode}</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-sm font-bold text-[#0a1a0f] ">{apt.date}</p>
          ...
          <div className="space-y-5">
            {vitals.map((v, i) => (
              <div key={v.label} className="flex items-center justify-between">
                ...
                <div className="text-right">
                  <span className="text-lg font-extrabold text-[#0a1a0f] ">{v.value}</span>
      ...
      <div className="finai-card p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {medications.map(med => (
            <div key={med.id} className="p-4 rounded-xl bg-slate-50/80 border border-slate-100 ">
              <h4 className="font-bold text-sm text-[#0a1a0f] ">{med.name}</h4>
              <p className="text-xs text-slate-500 mt-1">{med.instruction}</p>
              <p className="text-xs text-amber-600 font-semibold mt-2">Refill by: {med.refillDate}</p>
            </div>
          ))}
        </div>
      </div>
