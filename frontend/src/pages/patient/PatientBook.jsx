...
const PatientBook = () => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="space-y-6 p-4 lg:p-8 min-h-screen">
      ...
      <div>
        <h1 className="text-2xl lg:text-3xl font-extrabold text-slate-800 dark:text-white tracking-tight">Book Appointment</h1>
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Find a doctor and schedule your visit.</p>
      </div>

      {/* Progress Steps */}
      <div className="flex items-center gap-2 bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl border border-white/40 dark:border-slate-800 shadow-sm rounded-2xl p-4">
          ...
          <React.Fragment key={label}>
            <div className={cn("flex items-center gap-2", step > i + 1 ? "text-emerald-600" : step === i + 1 ? "text-blue-600" : "text-slate-400")}>
              <div className={cn("w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold border-2",
                step > i + 1 ? "bg-emerald-500 border-emerald-500 text-white" : step === i + 1 ? "border-blue-500 text-blue-600 dark:text-blue-400" : "border-slate-300 dark:border-slate-700")}>
                {step > i + 1 ? '✓' : i + 1}
              </div>
              <span className="text-sm font-semibold hidden sm:block">{label}</span>
            </div>
            {i < 2 && <div className={cn("flex-1 h-0.5 rounded-full", step > i + 1 ? "bg-emerald-500" : "bg-slate-200 dark:bg-slate-700")} />}
          </React.Fragment>
        ))}
      </div>
      ...
      {step === 1 && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
          <div className="bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl border border-white/40 dark:border-slate-800 shadow-sm rounded-2xl p-4 flex flex-col md:flex-row gap-4">
              ...
              {departments.map(dept => (
                <button key={dept} onClick={() => setSelectedDept(dept)}
                  className={cn("px-3 py-1.5 rounded-full text-xs font-bold border transition-all",
                    selectedDept === dept ? "bg-blue-600 text-white border-blue-600" : "bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-400 border-slate-200 dark:border-slate-700 hover:bg-slate-50")}>
                  {dept}
                </button>
              ))}
            </div>
            ...
            {filteredDoctors.map((doc, i) => (
              <motion.div key={doc.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                ...
                <div className="flex gap-4 mb-4">
                  <div className="w-14 h-14 rounded-full bg-gradient-to-tr from-blue-100 to-indigo-100 dark:from-slate-700 dark:to-slate-600 flex items-center justify-center font-bold text-xl text-blue-600 dark:text-blue-300 shrink-0">
                    {doc.name.charAt(4)}
                  </div>
                  <div className="flex-1">
                    <h3 className="font-bold text-slate-800 dark:text-white">{doc.name}</h3>
                  ...
                  <div className="text-right">
                    <p className="text-lg font-extrabold text-slate-800 dark:text-white">{doc.fee}</p>
                    <p className="text-[10px] text-slate-400 uppercase">per visit</p>
                  </div>
                </div>
                <div className="flex flex-wrap gap-1.5 mb-4">
                  {doc.modes.map(m => (
                    <span key={m} className="text-[10px] font-bold px-2 py-1 rounded-md bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 flex items-center gap-1">
      ...
      {step === 2 && selectedDoctor && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
          <div className="bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl border border-white/40 dark:border-slate-800 shadow-sm rounded-2xl p-6">
            ...
            <h2 className="text-lg font-bold text-slate-800 dark:text-white mb-1">Booking with {selectedDoctor.name}</h2>
                ...
                {selectedDoctor.modes.map(m => (
                  <button key={m} onClick={() => setSelectedMode(m)}
                    className={cn("px-5 py-2.5 rounded-xl text-sm font-bold border transition-all flex items-center gap-2",
                      selectedMode === m ? "bg-blue-600 text-white border-blue-600" : "bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300")}>
                ...
                {selectedDoctor.available.map(date => (
                  <button key={date} onClick={() => setSelectedDate(date)}
                    className={cn("px-5 py-3 rounded-xl text-sm font-bold border transition-all",
                      selectedDate === date ? "bg-blue-600 text-white border-blue-600 shadow-md" : "bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:bg-slate-50")}>
                    {date}
                  </button>
                ))}
              </div>
                  ...
                  {timeSlots.map(time => (
                    <button key={time} onClick={() => setSelectedTime(time)}
                      className={cn("py-2.5 rounded-xl text-sm font-bold border transition-all",
                        selectedTime === time ? "bg-blue-600 text-white border-blue-600 shadow-md" : "bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:bg-slate-50")}>
                      {time}
                    </button>
                  ))}
                </div>
      ...
      {step === 3 && selectedDoctor && (
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex items-center justify-center py-12">
          <div className="bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl border border-white/40 dark:border-slate-800 shadow-lg rounded-2xl p-10 text-center max-w-md">
            <div className="w-20 h-20 mx-auto bg-emerald-100 dark:bg-emerald-500/20 rounded-full flex items-center justify-center mb-6">
              <Calendar size={36} className="text-emerald-600 dark:text-emerald-400" />
            </div>
            <h2 className="text-2xl font-extrabold text-slate-800 dark:text-white mb-2">Booking Confirmed! 🎉</h2>
            <p className="text-slate-500 dark:text-slate-400 mb-6">Your appointment has been scheduled.</p>
            <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-5 text-left space-y-3 mb-8">
              <div className="flex justify-between"><span className="text-sm text-slate-500">Doctor</span><span className="text-sm font-bold text-slate-800 dark:text-white">{selectedDoctor.name}</span></div>
              <div className="flex justify-between"><span className="text-sm text-slate-500">Date</span><span className="text-sm font-bold text-slate-800 dark:text-white">{selectedDate}, 2026</span></div>
              <div className="flex justify-between"><span className="text-sm text-slate-500">Time</span><span className="text-sm font-bold text-slate-800 dark:text-white">{selectedTime}</span></div>
              <div className="flex justify-between"><span className="text-sm text-slate-500">Mode</span><span className="text-sm font-bold text-slate-800 dark:text-white">{selectedMode}</span></div>
              <div className="flex justify-between"><span className="text-sm text-slate-500">Fee</span><span className="text-sm font-bold text-slate-800 dark:text-white">{selectedDoctor.fee}</span></div>
            </div>
