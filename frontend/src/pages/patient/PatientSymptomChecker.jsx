      ...
      <div className="bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl border border-white/40 dark:border-slate-800 shadow-sm rounded-2xl p-4 flex items-center justify-between gap-4 overflow-x-auto">
        {[
          ...
          <div key={s.num} className="flex items-center gap-2 text-xs font-bold shrink-0">
            ...
            {s.num < 4 && <ChevronRight size={14} className="text-slate-350" />}
          </div>
        ))}
      </div>
        ...
        <AnimatePresence mode="wait">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {bodyRegions.map((region) => (
                  <button 
                    ...
                    className="p-5 text-left bg-slate-50 hover:bg-slate-100 dark:bg-slate-950/60 dark:hover:bg-slate-900 border border-slate-200 dark:border-slate-850 rounded-2xl transition-all shadow-sm hover:shadow-md cursor-pointer group"
                  >
            ...
            <motion.div key="step2" initial={{ opacity: 0, x: 15 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -15 }} className="space-y-6">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs font-semibold text-slate-450">Check all indicators matching your current feeling.</p>
                </div>
              </div>
                      ...
                      className={cn("px-4 py-2.5 rounded-xl border text-xs font-bold transition-all cursor-pointer", 
                        isChecked 
                          ...
                          : s.warning 
                            ...
                            : "bg-slate-50 border-slate-200 dark:bg-slate-950 dark:border-slate-850 text-slate-650 dark:text-slate-350 hover:bg-slate-100"
                      )}
                    >
                      {s.label} {s.warning && "⚠️"}
                    </button>
            ...
            <motion.div key="step3" initial={{ opacity: 0, x: 15 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -15 }} className="space-y-6">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs font-semibold text-slate-450">These questions assist in calculating standard severity levels.</p>
                </div>
              </div>
                        ...
                        return (
                          <button
                            ...
                            className={cn("py-2.5 px-4 text-center rounded-xl border text-xs font-bold transition-all cursor-pointer", 
                              isChosen 
                                ? "bg-slate-800 border-slate-900 text-white dark:bg-slate-750 dark:border-slate-700" 
                                : "bg-slate-50 border-slate-200 dark:bg-slate-950 dark:border-slate-850 text-slate-600 dark:text-slate-350 hover:bg-slate-100"
                            )}
                          >
                            {opt}
                          </button>
          ...
          {step === 4 && (
            <motion.div key="step4" initial={{ opacity: 0, x: 15 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -15 }} className="space-y-6">
              <div className="p-6 rounded-2xl border flex flex-col md:flex-row gap-5 items-start relative overflow-hidden bg-slate-50 dark:bg-slate-950/60 border-slate-200 dark:border-slate-850">
                ...
                <div className="space-y-2.5">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="text-[10px] font-extrabold uppercase tracking-widest bg-slate-200 dark:bg-slate-900 px-3 py-1 rounded-full text-slate-550 dark:text-slate-400">Clinical Triage Score</span>
                  ...
                  <p className="text-xs font-semibold text-slate-500 dark:text-slate-450 leading-relaxed">{assessment.desc}</p>
                </div>
              </div>

              {/* Action grid */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-4 border-t border-slate-100 dark:border-slate-800">
                <button onClick={handleReset} className="py-3 border border-slate-250 hover:bg-slate-50 dark:border-slate-800 dark:hover:bg-slate-900 text-slate-700 dark:text-slate-300 font-bold text-xs uppercase tracking-wider rounded-xl transition-all">
