...
const DoctorPatients = () => {
  ...
  return (
    ...
    >
      {/* Header */}
      <div>
        <h1 className="text-2xl lg:text-3xl font-extrabold text-[#0a1a0f] tracking-tight">Patient Directory</h1>
        <p className="text-sm text-slate-500 mt-1">Manage and view details for all patients assigned under your care.</p>
      </div>
        ...
        {filteredPatients.length > 0 ? (
          filteredPatients.map((p, i) => (
            <motion.div 
              ...
              className="bg-white border-[#e2e8e2] shadow-sm hover:shadow-md rounded-[24px] p-6 flex flex-col justify-between transition-all relative overflow-hidden group"
            >
              {/* Blood Group indicator tag */}
              <div className="absolute right-4 top-4 bg-blue-50 border border-blue-100 text-blue-600 text-xs font-bold px-2.5 py-1 rounded-lg">
                BG: {p.bloodGroup}
              </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-3.5">
                  <div className="w-12 h-12 rounded-[24px] bg-gradient-to-tr from-blue-500 to-indigo-500 flex items-center justify-center text-white font-extrabold text-lg shadow-md shadow-blue-500/15">
                    {p.name.charAt(0)}
                  </div>
                  <div>
                    <h3 className="font-extrabold text-[#0a1a0f] leading-tight">{p.name}</h3>
                ...
                <div className="space-y-2 border-t border-slate-100 pt-4 text-xs font-semibold text-slate-500 ">
                  <div className="flex items-center gap-2">
                    ...
                    <span>Primary Diagnosis: <strong className="text-slate-700 ">{p.condition}</strong></span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Calendar size={14} className="text-slate-400" />
                    <span>Last Consulted: <span className="font-mono text-slate-600 ">{p.lastVisit}</span></span>
                  </div>
                  <div className="flex items-center gap-2">
              ...
              <div className="grid grid-cols-2 gap-2 mt-6 border-t border-slate-100 pt-4">
                <button
                  ...
                  className="py-2.5 bg-slate-50 hover:bg-slate-100 :bg-slate-800 text-slate-700 rounded-xl text-xs font-bold border border-slate-200 transition-colors flex items-center justify-center gap-1.5"
                >
          ...
          <div className="col-span-full p-12 text-center text-slate-400 border-2 border-dashed border-slate-200 rounded-[24px]">
