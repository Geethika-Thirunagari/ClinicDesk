              ...
              {queue.map((q, i) => (
                <motion.tr key={q.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }}
                  className={cn("border-b border-slate-100 transition-colors",
                    q.status === 'In Progress' ? "bg-blue-50/50 " : "hover:bg-slate-50/50")}>
                  <td className="px-6 py-4 text-sm font-mono font-bold text-blue-600 ">{q.token}</td>
                  <td className="px-6 py-4 text-sm font-bold text-[#0a1a0f] ">{q.patient}</td>
                  <td className="px-6 py-4 text-sm text-slate-600 ">{q.doctor}</td>
                  <td className="px-6 py-4 text-sm text-slate-600 ">{q.time}</td>
