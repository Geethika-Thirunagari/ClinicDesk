...
const initialDoctorsFallback = [
  { id: 1, name: "Dr. Sarah Smith", specialty: "Cardiologist", status: "Active" },
  { id: 2, name: "Dr. John Doe", specialty: "Neurologist", status: "Active" },
  { id: 3, name: "Dr. Emily Chen", specialty: "Dermatologist", status: "Active" },
];
...
export default function ReceptionCheckin() {
  const [patientsDatabase, setPatientsDatabase] = useState(() => {
    ...
    return saved ? JSON.parse(saved) : initialPatientsFallback;
  });

  const [doctorsList, setDoctorsList] = useState(() => {
    const saved = localStorage.getItem('clinicdesk_doctors');
    return saved ? JSON.parse(saved) : initialDoctorsFallback;
  });
  ...
  const handleSearch = (e) => {
    ...
    // Look up in actual database
    const results = patientsDatabase.filter(p =>
  ...
  const handleProcessCheckin = () => {
    ...
    setTimeout(() => {
      ...
      // Save to Shared Queue Registry
      const currentQueue = JSON.parse(localStorage.getItem('clinicdesk_queue') || '[]');
      const newEntry = {
        id: Date.now(),
        token: token,
      ...
      localStorage.setItem('clinicdesk_queue', JSON.stringify([newEntry, ...currentQueue]));

      // Update patient last visit in database
      const updatedPatients = patientsDatabase.map(p =>
        p.id === selectedPatient.id ? { ...p, lastVisit: new Date().toISOString().split('T')[0] } : p
      );
      setPatientsDatabase(updatedPatients);
      localStorage.setItem('clinicdesk_patients', JSON.stringify(updatedPatients));
                    ...
                    >
                      <div className="flex items-center gap-3.5">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-400 mt-1 font-mono">{p.id} • {p.age} yrs • {p.gender}</p>
                        </div>
                      </div>
                ...
                <div>
                  ...
                  >
                    ...
                    {doctorsList.map((d, idx) => (
                      <option key={d.id || idx} value={d.name}>{d.name} ({d.specialty})</option>
                    ))}
                  </select>
                </div>
                <div>
