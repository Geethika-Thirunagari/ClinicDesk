...
const ReceptionBooking = () => {
    ...
    return (
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex items-center justify-center p-8 min-h-screen">
        <div className="bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl border border-white/40 dark:border-slate-800 shadow-lg rounded-2xl p-10 text-center max-w-md">
          ...
          <h2 className="text-2xl font-extrabold text-slate-800 dark:text-white mb-2">Appointment Booked! 🎉</h2>
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 p-4 lg:p-8 min-h-screen">
      ...
      <div className="bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl border border-white/40 dark:border-slate-800 shadow-sm rounded-2xl p-6 space-y-6">
        {/* Patient Search */}
        <div>
            ...
            {doctors.map(doc => (
              <button key={doc.id} onClick={() => { setSelDoc(doc); setSelSlot(null); }}
                className={cn("p-4 rounded-xl border text-left transition-all",
                  selDoc?.id === doc.id ? "bg-blue-50 border-blue-300 dark:bg-blue-500/10 dark:border-blue-500/30 shadow-sm" : "bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 hover:bg-slate-50")}>
              ...
              {selDoc.slots.map(slot => (
                <button key={slot} onClick={() => setSelSlot(slot)}
                  className={cn("px-5 py-2.5 rounded-xl text-sm font-bold border transition-all",
                    selSlot === slot ? "bg-blue-600 text-white border-blue-600 shadow-md" : "bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300")}>
